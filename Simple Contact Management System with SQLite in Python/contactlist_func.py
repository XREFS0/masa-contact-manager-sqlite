...
def add_person(app):
    ...
    insert_row = (
        app.text_first_name.get(),
        app.text_last_name.get(),
        app.text_email.get(),
        app.text_phone_number.get(),
        app.text_address.get(),
    )
    conn = db_conn()
    with conn:
        cur = conn.cursor()
        sql_query = (
            "INSERT INTO tbl_contactlist (col_fname, col_lname, col_email, col_phone, col_address)"
            "VALUES (?, ?, ?, ?, ?)"
        )
...
def edit_person(app):
    ...
    with conn:
        cur = conn.cursor()
        sql_query = (
            "UPDATE tbl_contactlist SET col_fname=?, col_lname=?, col_email=?, col_phone=?, col_address=?"
            "WHERE ID=?"
        )
...
def create_db():
    ...
    with conn:
        cur = conn.cursor()
        sql_query = (
            "CREATE TABLE IF NOT EXISTS tbl_contactlist ("
